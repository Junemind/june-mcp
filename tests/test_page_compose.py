...
class TestPureSerializers(unittest.TestCase):
    ...
    def test_media_image_vs_link_vs_unsafe(self) -> None:
        ...
        # `place` rides the title slot; a stray quote can never end the URL early
        self.assertEqual(_media_text({"type": "image", "url": ref, "alt": "logo", "place": "w=60% align=center"}),
                         f'![logo]({ref} "w=60% align=center")')
        self.assertEqual(_media_text({"type": "image", "url": ref, "place": 'w=60" onerror=x'}),
                         f'![]({ref} "w=60  onerror=x")')
