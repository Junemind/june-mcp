...
SERVER_INSTRUCTIONS = (
    ...
    "june_page_write must grow out of a june_page_get made in the same turn: build the blocks "
    "from what came back and carry their ids (a payload with ids demonstrably grew out of a real "
    "read; concurrency is handled for you, there is no token to pass). A write that would remove "
    "10 or more blocks is refused and names them; only a deliberate `force: true` replaces wholesale. Prefer june_page_append — it cannot "
    ...
    "delete anything, so a large page never has to be resent to change a few lines. Place new "
    "blocks at a position with june_page_insert (after a named block), reorder with "
    "june_page_move, retitle with june_page_rename; if a write lost something, june_page_restore "
    "puts it back with its original ids (june_page_removed lists what went) — never re-type it. "
    "None of these resends the page. Use "
    "june_page_write only when the user asked to replace, rewrite or "
    "restructure; 'update' usually means add.\n"
    "\n"
    "Canvas targeting is per call (CX3): your "
